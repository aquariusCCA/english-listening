...
| 指令 | 動作 | 規則所在章節 |
|---|---|---|
| `/pick` | 階段一：從逐字稿挑出中難度字彙，列表後停下等挑選 | 〈階段一：提取中難度字彙〉 |
| `/cards` | 階段二：出 CSV、更新 README 目錄、歸檔逐字稿 | 〈階段二：產出 Anki CSV〉 |
| `/add` | 補卡：使用者指名詞條，直接追加到既有 CSV | 〈補卡：追加詞條到既有 CSV〉 |
| `/recheck` | 補卡：重掃逐字稿，只列尚未收錄的詞條 | 〈補卡：追加詞條到既有 CSV〉 |
| `/end` | 收尾：commit + push | 〈Git 工作流程〉 |
