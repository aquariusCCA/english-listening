...
| 指令 | 動作 |
|---|---|
| `/pick` | 從逐字稿挑出中難度字彙，列表後停下等挑選 |
| `/cards` | 依挑選編號出 CSV、更新 README 目錄、歸檔逐字稿 |
| `/add` | 補卡：使用者指名詞條，直接追加到既有 CSV |
| `/recheck` | 補卡：重掃逐字稿，只列尚未收錄的詞條 |
| `/end` | 收尾：commit + push |

- 主流程是 `/pick` 挑字 → 使用者回覆編號 → `/cards` 出卡；複習後發現漏字才走 `/add` 或 `/recheck`。
- 各指令的規則在下方「動作規則」一節**以指令為名的章節**；`/end` 無獨立章節，規則即〈Git 工作流程〉的收尾條目。
- `.claude/commands/` 內的指令檔只是**薄指標**，規則本體在本檔各章節，指令檔中不重複規則內容——改規則只需改本檔一處。
- 與 grammar-in-charts 不同，本專案**不另設 `rules/` 資料夾**：動作只有五個、規則量小，全放本檔即可；拆檔就多一處要同步維護的地方（同「不另開 INDEX.md」的原則）。
- 指令不是必要入口：使用者未下指令但意圖明確時（例如直接貼上逐字稿），依對應章節執行即可，不必要求他先打指令。
...
.claude/commands/                     動作選單的指令檔（薄指標，見「動作選單」）
...
- **`/recheck`——使用者要你「再檢查有無遺漏」** → 重跑 `/pick` 的挑字流程，但**只列出該 CSV 尚未收錄的詞條**，照常編號後停下等他挑選。
